    ...
    publicMethods = {
        pushTag : function (tag, ignoreEvents, externalTagId) {
            ...
            if (alreadyInList) {
                ...
                html = '<span class="' + privateMethods.tagClasses.call($self) + ' bg-gradient" id="' + newTagId + '">';
                html+= '<span>' + escaped + '</span>';
                html+= '<input name="priceee[]" type="text" placeholder="Price Change" title="Define how the price change, e.g. `+5000` (add price by 5000), `*2-1000` (double price then minus 1000), etc" data-toggle="tooltip" data-placement="bottom" style="width:100px;margin-left:8px;color:#000;font-weight:normal">';
                html+= '<a class="tm-tag-remove" id="' + newTagRemoveId + '" TagIdToRemove="' + tagId + '">';
                html+= opts.tagCloseIcon + '</a></span> ';
                $el = $(html);

                if (opts.tagsContainer !== null) {
                    //$(opts.tagsContainer).append($el);
                    $self.parent().find('.tag-container').append($el);
                } else {
                    if (tlid.length > 1) {
    ...
    privateMethods = {
        ...
        init : function (options) {
            ...
            this.each(function() {
				...
				var hi = (opts.hiddenTagListName === null)
					? 'tag_' + $self.attr('name')
					: opts.hiddenTagListName;
